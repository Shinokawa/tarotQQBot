...
# 旋转逆位塔罗牌图片
def rotate_image_if_reversed(image_path):
    try:
        with Image.open(image_path) as img:
            rotated_img = img.rotate(180)  # 旋转180度表示逆位
...
def create_combined_image(tarot_card, interpretation, image_path, output_path):
    try:
        # 加载塔罗牌图片
        with Image.open(image_path) as card_img:
            ...
            # 在下部绘制塔罗牌图片
            card_position = ((new_width - card_width) // 2, 0)  # 下部居中绘制图片
...
def format_tarot_message_with_image(tarot_card, image_base_path, output_base_path):
    if tarot_card:
        ...
        if is_upright:
            ...
            image_path = rotate_image_if_reversed(image_path)  # 逆位旋转图片
        ...
        combined_image_path = create_combined_image(tarot_card, interpretation_message, image_path, output_image_path)
